...
class AccessoryDemoViewController: UIViewController {
    ...
    let logger = os.Logger(subsystem: "it.truesense.NINearbyAccessorySample", category: "AccessoryDemoViewController")
    ...
    func accessorySharedData(data: Data, accessoryName: String, peerID: UUID) {
        ...
        case .accessoryConfigurationData:
            ...
            setupAccessory(message, name: accessoryName,peerID: peerID)
    ...
    func setupAccessory(_ configData: Data, name: String, peerID: UUID) {
        updateInfoLabel(with: "Received configuration data from '\(name)'. Running session.")
        do {
            if(configData.count==37)
            {
                configuration = try NINearbyAccessoryConfiguration(accessoryData: configData, bluetoothPeerIdentifier: peerID)
                updateInfoLabel(with: "NIConfiguration OK")
            }
...
extension AccessoryDemoViewController: NISessionDelegate {
    ...
    func session(_ session: NISession, didUpdate nearbyObjects: [NINearbyObject]) {
        ...
        if let angle = accessory.horizontalAngle {
            self.distanceLabel.text = String(format: "%0.1fm at %0.1f°",distance, 180 * angle / .pi)
        } else {
            self.distanceLabel.text = String(format: "%0.1fm", distance)
        }
    ...
    func session(_ session: NISession, didUpdateAlgorithmConvergence convergence: NIAlgorithmConvergence, for object: NINearbyObject?){
        logger.info("updated Algorithm Convergence")
    }
